...
| Field | Rule |
|---|---|
| **Owner** | Must map to a project agent. Skip founder-only items — no task file. |
...
Combine related engineering items into one task only when they share the same owner, dependency chain, and execution slot.
