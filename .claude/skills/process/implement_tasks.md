...
description: >-
  ...
  appropriate project agents in dependency order. Usage: /implement-tasks
---
...
Parse flags and positional args from `$ARGUMENTS`.
...
For each task in the current wave, spawn the **Owner** agent using the **Agent** tool.

Read `.claude/agents/<owner>.md` when the agent definition exists. Available agents: `ceo`, `cto`, `cpo`, `cgo`, `devops`, `nextjs-frontend-developer`, `ux-ui-designer`, `test-runner`, `agentic-architect`, `azure-cloud-developer`, `cold-outbound-sdr`, `code-commit`.
...
Run agents in a wave **in parallel** when independent. Wait for all to complete before the next wave.
...
2. Append to `business/tasks/progress.md`:
   ...
   ```
3. If `--with-tests` and Owner is a code agent: spawn `test-runner` for changed files
...
If `--commit`: after each successful task, spawn `code-commit` to stage, commit, and push.
...
- **Owner** is `Founder` or any non-implementing role without a code deliverable
- **Status** is already `Done`, `Blocked`, or `Cancelled`
- Dependencies are unresolved

## Reference

- Task file format: `.claude/skills/process/task_file_template.md`
- Agent definitions: `.claude/agents/<owner>.md`
