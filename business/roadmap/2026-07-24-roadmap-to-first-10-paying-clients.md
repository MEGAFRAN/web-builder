...
| Founder bandwidth | **~20 hours/week** (2 jobs). Every process must be no-discovery-call, no-manual-code, script-driven. |
...
**Acceptance gate:** `CLIENT_ID=demo-phone-repair-shop npm run build:blob` with empty env → priced services homepage with WhatsApp CTAs → **Actions → Deploy Demo Client - SWA** → live at `https://moviles.clubtal.com`.
