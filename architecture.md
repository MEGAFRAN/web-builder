...
**→ Agent-facing component catalog (auto-generated): [`docs/agents/component-catalog.md`](docs/agents/component-catalog.md)** — merges `componentRegistry.ts`, `types/cms.ts`, schemas, primitives, and **`config/component-affordances.json`** (when-to-use copy). Run `npm run generate:component-catalog` after changes.
