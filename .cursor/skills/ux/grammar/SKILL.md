...
name: grammar
description: >-
  Fixes grammar, spelling, and punctuation in a file for a specified language.
  Use when the user invokes /grammar with a language and file path, or asks for
  proofreading of UI copy or prose in a specific file.
...
Parse the user's message after `/grammar`:
...
- Read `file` with Read before changing it — do not invent content or "fix" from memory.
...
4. **Apply**  
   Write edits back using StrReplace or Write. Keep the diff minimal and limited to `file`.
